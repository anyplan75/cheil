...
JIFC.broadcast = (() => {
  ...
  function saveApiKey(key) {
    ...
    localStorage.setItem("cheil_openai_key", trimmed);
  }

  function loadApiKey() {
    state.apiKey = localStorage.getItem("cheil_openai_key") || "";
    return state.apiKey;
  }

  function saveModel(model) {
    state.model = model;
    localStorage.setItem("cheil_openai_model", model);
  }

  function loadModel() {
    state.model = localStorage.getItem("cheil_openai_model") || JIFC.config.defaultModel;
    return state.model;
  }
  ...
  const TARGETS_KEY = "cheil_broadcast_targets";
